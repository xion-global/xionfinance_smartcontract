...
contract XGTStakeXDai is Initializable, OpenZeppelinUpgradesOwnable {
    ...
    function initialize(address _bridge, address _stakingContractMainnet, address _xgt, uint256 _initialXGTGenerationRate) public initializer {
    ...
    function updateXGTGenerationRate(uint256 _newXGTGenerationRate) public onlyOwner {
        xgtGenerationRate = _newXGTGenerationRate;
    }

    function tokensDeposited(uint256 _amount, address _user) external notPaused {
        require(msg.sender == address(bridge), "XGTSTAKE-NOT-BRIDGE");
        require(bridge.messageSender() == stakingContractMainnet, "XGTSTAKE-NOT-MAINNET-CONTRACT");
        ...
        if(specificUserDeposits[_user].deposits[lastItem].generationRate == xgtGenerationRate) {
            specificUserDeposits[_user].deposits[lastItem].amount = specificUserDeposits[_user].deposits[lastItem].amount.add(_amount);
        } else {
            specificUserDeposits[_user].deposits.push(DepositPerRate(_amount, xgtGenerationRate));
        }
    }

    function tokensWithdrawn(uint256 _amount, address _user) external notPaused {
        require(msg.sender == address(bridge), "XGTSTAKE-NOT-BRIDGE");
        require(bridge.messageSender() == stakingContractMainnet, "XGTSTAKE-NOT-MAINNET-CONTRACT");
        ...
        for(uint256 i = specificUserDeposits[_user].deposits.length-1; i >= 0; i--) {
            // If the amount in this entry is enough, subtract as much as needed and set remainder to zero
            if(specificUserDeposits[_user].deposits[i].amount >= remainingAmount) {
                specificUserDeposits[_user].deposits[i].amount = specificUserDeposits[_user].deposits[i].amount.sub(remainingAmount);
                remainingAmount = 0;
            // If the amount is not enough, take the amount and continue
            } else {
                specificUserDeposits[_user].deposits[i].amount = 0;
                remainingAmount = remainingAmount.sub(specificUserDeposits[_user].deposits[i].amount);
            }
            ...
            if(specificUserDeposits[_user].deposits[i].amount == 0) {
                ...
                specificUserDeposits[_user].deposits.length = specificUserDeposits[_user].deposits.length.sub(1);
            }
        ...
        require(remainingAmount == 0, "XGTSTAKE-WITHDRAW-NOT-EXECUTED-CORRECTLY");
    }
    ...
    function GetUnclaimedXGT(address _user) public view returns(uint256) {
        ...
        for(uint256 i = 0; i < specificUserDeposits[_user].deposits.length; i++) {
            unclaimedXGT = unclaimedXGT.add(specificUserDeposits[_user].deposits[i].amount.mul(specificUserDeposits[_user].deposits[i].generationRate).mul(diffTime));
        }
