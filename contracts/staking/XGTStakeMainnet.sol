...
contract XGTStakeMainnet is Initializable, OpenZeppelinUpgradesOwnable {
    ...
    uint256 public interestCut = 1000; // Interest Cut in Basis Points (1000 = 10%)
    ...
    function initialize(address _stakeToken, address _cToken, address _bridge, address _stakingContractXdai) public initializer {
        stakeToken = IPERC20(_stakeToken);
        cToken = ICToken(_cToken);
        bridge = IBridgeContract(_bridge);
        gasOracle = IChainlinkOracle(0x169E633A2D1E6c10dD91238Ba11c4A708dfEF37C);
        ethDaiOracle = IChainlinkOracle(0x773616E4d11A78F511299002da57A0a94577F1f4);
        stakingContractXdai = _stakingContractXdai;
    }

    function changeMetaTxAuth(address _user, bool _allowedToExecute) external onlyOwner {
    ...
    function depositTokensForUser(uint256 _amount, address _user) external notPaused {
    ...
    function _depositTokens(uint256 _amount, address _user) internal {
        require(stakeToken.transferFrom(_user, address(this), _amount), "XGTSTAKE-DAI-TRANSFER-FAILED");
    ...
    function withdrawTokensForUser(uint256 _amount, address _user) external notPaused {
        require(metaTransactors[msg.sender], "XGTSTAKE-NOT-ALLOWED");
    ...
    function _withdrawTokens(uint256 _amount, address _user) internal {
        ...
        if(_amount != cDaiToRedeem) {
            cDaiToRedeem = (((_amount.mul(1000)).div(userDepositDai)).mul(userDepositCDai)).div(1000);
        }
        ...
        require(cToken.redeem(cDaiToRedeem) == 0, "XGTSTAKE-COMPOUND-WITHDRAW-FAILED");
        ...
        uint256 cut = ((diff.sub(amount)).mul(interestCut)).div(1000);
        stakeToken.transferFrom(address(this), interestCutReceiver, cut);

        // Transfer the rest to the user
        stakeToken.transferFrom(address(this), _user, diff.sub(cut));
    ...
    function refundGas(uint256 _amount, address _user, uint256 _gasAmount) internal returns(uint256) {
        int256 latestGasPrice = gasOracle.latestAnswer();
        uint256 latestEthPrice = uint256(1 ether).div(uint256(ethDaiOracle.latestAnswer()));
        uint256 amount = _amount;
        if(latestGasPrice >= 0 && latestEthPrice >= 0) {
            uint256 refund = uint256(latestGasPrice).mul(_gasAmount).mul(latestEthPrice);
            require(refund < _amount, "XGTSTAKE-DEPOSIT-TOO-SMALL");
            amount = _amount.sub(refund);
            require(stakeToken.transferFrom(_user, msg.sender, refund), "XGTSTAKE-DAI-REFUND-TRANSFER-FAILED");
        }
        return amount;
    }
